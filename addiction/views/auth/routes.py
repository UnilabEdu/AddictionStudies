...
def resend_confirmation():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            resend_key=create_key(form.email.data)
            html=render_template('auth/_activation_message.html', key=resend_key)
            send_email('ანგარიშის დადასტურება', html, form.email.data)
            user.save()
            return "ანგარიშის დადასტურების ლინკი გამოგზავნილია თქვენს ელფოსტაზე"
